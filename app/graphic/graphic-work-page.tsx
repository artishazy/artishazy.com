...
export default function GraphicWorkPage({slug}:{slug:keyof typeof descriptions}){
  ...
  return <LocalizedShell>{ru=><>
    <section className="art-detail-hero"><div className="kicker">03—{ru?"ГРАФИКА":"GRAPHIC"} / {work.id}</div><h1>{ru?work.titleRu:work.titleEn}</h1><p>{ru?descriptions[slug].ru:descriptions[slug].en}</p></section>
    ...
    <section className="art-detail-copy"><span className="kicker">{ru?category.ru:category.en} / {work.year}</span><p>{ru?"Страница подготовлена для полноценного кейса: сюда можно добавить задачу, процесс, варианты, финальные носители и изображения проекта.":"This page is ready for a full case: brief, process, iterations, final applications and project imagery."}</p></section>
  </>}</LocalizedShell>;
}
