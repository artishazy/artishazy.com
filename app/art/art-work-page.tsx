...
export default function ArtWorkPage({slug}:{slug:keyof typeof copy}){
  ...
  return <LocalizedShell>{ru=><>
    <section className="art-detail-hero"><a className="case-back" href="/art"><ArrowIcon direction="left"/>{ru?"ВСЯ АРТ-ЛАБОРАТОРИЯ":"ALL ART WORKS"}</a><div className="kicker">05—{ru?"АРТ-ЛАБОРАТОРИЯ":"ART LAB"}</div><h1>{ru?work.titleRu:work.titleEn}</h1><p>{ru?copy[slug].ru:copy[slug].en}</p></section>
